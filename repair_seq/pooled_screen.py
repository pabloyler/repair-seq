...
class SingleGuideExperiment(experiment.Experiment):
        ...
        self.fns.update(
            {
                ...
                "genomic_insertion_seqs": self.results_dir
                / "genomic_insertion_seqs.fa",
                "filtered_genomic_insertion_seqs": self.results_dir
                / "filtered_genomic_insertion_seqs.fa",
                "filtered_genomic_insertion_details": self.results_dir
                / "filtered_genomic_insertion_details.txt",
                "filtered_templated_insertion_details": self.results_dir
                / "filtered_templated_insertion_details.hdf5",
                "filtered_duplication_details": self.results_dir
                / "filtered_duplication_details.hdf5",
                "deletion_ranges": self.results_dir / "deletion_ranges.hdf5",
                "duplication_ranges": self.results_dir / "duplication_ranges.hdf5",
                "collapsed_UMI_outcomes": self.results_dir
                / "collapsed_UMI_outcomes.txt",
                "cell_outcomes": self.results_dir / "cell_outcomes.txt",
                "filtered_cell_outcomes": self.results_dir
                / "filtered_cell_outcomes.txt",
                ...
                "common_sequence_outcomes": self.results_dir
                / "common_sequences"
                / "common_sequence_outcomes.txt",
                "common_sequence_special_alignments": self.results_dir
                / "common_sequences"
                / "all_special_alignments.bam",
            }
        )
    ...
    def target_info(self):
        protospacer_sequence = self.pool.variable_guide_library.guides_df.loc[
            self.variable_guide, "protospacer"
        ]
    ...
    def get_read_alignments(
        self, read_id, fn_key="bam_by_name", outcome=None, read_type=None
    ):
        ...
        if not looked_up_common:
            als = super().get_read_alignments(
                read_id, fn_key=fn_key, outcome=outcome, read_type=read_type
            )
    ...
    def collapse_UMI_reads(self):
        ...
        def UMI_key(read):
            return annotations.Annotations["UMI_guide"].from_identifier(read.name)[
                "UMI"
            ]

        def num_reads_key(read):
            return annotations.Annotations["collapsed_UMI"].from_identifier(read.name)[
                "num_reads"
            ]
        ...
        expected_seq = self.pool.variable_guide_library.guides_df.loc[
            self.variable_guide, "full_seq"
        ][:R1_read_length].upper()
        ...
        with gzip.open(collapsed_fn, "wt", compresslevel=1) as collapsed_fh:
            ...
            for UMI, UMI_group in groups:
                clusters = collapse.form_clusters(
                    UMI_group, max_read_length=None, max_hq_mismatches=0
                )
                ...
                for i, cluster in enumerate(clusters):
                    ...
                    if annotation["num_reads"] >= self.min_reads_per_cluster:
                        ...
                        if guide == expected_seq:
                            ...
                            mismatches = []
                            for i, (seen, expected, q) in enumerate(
                                zip(guide, expected_seq, qs)
                            ):
                        ...
                        mismatch_annotation = annotations.Annotations[
    ...
    def common_sequence_experiment(self):
        ...
        return CommonSequenceExperiment(
            self.base_dir,
            self.pool_name,
            self.fixed_guide,
            self.variable_guide,
    ...
    def common_name_to_special_alignment(self):
        ...
        if self.fns["common_sequence_special_alignments"].exists():
            for al in pysam.AlignmentFile(
                self.fns["common_sequence_special_alignments"]
            ):
    ...
    def combined_header(self):
        return sam.get_header(
            self.fns_by_read_type["bam_by_name"]["collapsed_uncommon_R2"]
        )
    ...
    def categorize_outcomes(self, max_reads=None):
        ...
        alignment_groups = self.alignment_groups(
            fn_key="bam_by_name", read_type=bam_read_type
        )
        ...
        with self.fns["outcome_list"].open("w") as outcome_fh, self.fns[
            "genomic_insertion_seqs"
        ].open("w") as genomic_insertion_seqs_fh:

            outcome_fh.write(f"## Generated at {utilities.current_time_string()}\n")

            for read in self.progress(reads, desc="Categorizing reads"):
                if (
                    self.use_memoized_outcomes
                    and read.name in self.qname_to_common_name
                ):
                    common_sequence_name = self.qname_to_common_name[read.name]
                    layout = self.common_name_to_outcome[common_sequence_name]
                    special_alignment = self.common_name_to_special_alignment.get(
                    ...
                    if layout.outcome is not None:
                        ...
                        layout.details = str(
                            layout.outcome.perform_anchor_shift(self.target_info.anchor)
                        )
                ...
                if isinstance(self, CommonSequenceExperiment):
                    ...
                    outcome = coherence.Pooled_UMI_Outcome(
                        annotation["UMI"],
                        annotation["mismatch"],
                    ...
                    outcome = coherence.gDNA_Outcome.from_layout(
                        layout,
                ...
                if (
                    layout.category == "genomic insertion"
                    and layout.subcategory == "hg19"
                ):
        ...
        with alignment_sorters:
            ...
            with pysam.AlignmentFile(bam_fn) as full_bam_fh:
                for al in self.progress(
                    full_bam_fh, desc="Making outcome-specific bams"
                ):
    ...
    def generate_outcome_counts(self):
        ...
        with open(counts_fn, "w") as fh:
            for fn_key, metadata_lines in self.outcome_metadata(
                outcome_fn_keys=outcome_fn_keys
            ):
    ...
    def collapse_UMI_outcomes(self):
        ...
        ) = coherence.collapse_pooled_UMI_outcomes(outcome_iter)
    ...
    def make_filtered_cell_bams(self):
        ...
        for _, row in cells.query("guide_mismatch == -1").iterrows():
            name_to_outcome[row["original_name"]] = (
                row["category"],
                row["subcategory"],
            )

        outcomes_seen = (
            cells.groupby(by=["category", "subcategory"]).size().index.values
        )
        ...
        with pysam.AlignmentFile(bam_fn) as combined_bam_fh:
            ...
            with sorters:
                for alignment in self.progress(
                    combined_bam_fh, desc="Making filtered cell bams"
                ):
    ...
    def make_reads_per_UMI(self, individual_outcomes=None):
        ...
        with open(self.fns["cell_outcomes"]) as fh:
            for line in fh:
                ...
                reads_per_UMI[outcome.category, outcome.subcategory][
                    outcome.num_reads
                ] += 1
    ...
    def cell_outcomes(self):
        df = pd.read_csv(
            self.fns["cell_outcomes"],
            header=None,
            na_filter=False,
            names=coherence.Pooled_UMI_Outcome.columns,
            sep="\t",
        )
        return df
    ...
    def filtered_cell_outcomes(self):
        df = pd.read_csv(
            self.fns["filtered_cell_outcomes"],
            header=None,
            na_filter=False,
            names=coherence.Pooled_UMI_Outcome.columns,
            sep="\t",
    ...
    def get_read_diagram(
        self, read_id, relevant=True, qname_to_als=None, outcome=None, **diagram_kwargs
    ):
        layout = self.get_read_layout(
            read_id, qname_to_als=qname_to_als, outcome=outcome
        )
    ...
    def generate_length_range_figures(self, specific_outcome=None, num_examples=1):
        ...
        length_ranges = [
            ...
            for _, row in self.length_ranges(specific_outcome).iterrows()
        ]
        ...
        for outcome in self.progress(outcomes, desc="Populating length ranges"):
            ...
            if length >= self.max_relevant_length:
                ...
                if last_range.start == self.max_relevant_length:
                    by_length_range[last_range.start, last_range.end].add(
                        (outcome.query_name, als)
                    )
                else:
                    raise ValueError
            else:
        ...
        items = self.progress(
            by_length_range.items(), desc=description, total=len(by_length_range)
        )

        for (start, end), sampler in items:
            diagrams = self.alignment_groups_to_diagrams(
                sampler.sample,
    ...
    def extract_truncation_positions(self):
        ...
        for outcome in self.outcome_iter():
            if (
                outcome.category == "truncation"
                and outcome.guide_mismatch == -1
                and outcome.details != "None"
            ):
    ...
    def extract_templated_insertion_info(self):
        ...
        for outcome in self.outcome_iter(outcome_fn_keys=["filtered_cell_outcomes"]):
            if (
                outcome.category in relevant_categories
                and getattr(outcome, "guide_mismatch", -1) == -1
            ):
                insertion_outcome = (
                    pooled_layout.LongTemplatedInsertionOutcome.from_string(
                        outcome.details
                    )
                )
        ...
        with h5py.File(
            self.fns["filtered_templated_insertion_details"], "w"
        ) as hdf5_file:
    ...
    def extract_duplication_info(self):
        ...
        with open(self.fns["filtered_cell_outcomes"]) as outcomes_fh:
            for line in outcomes_fh:
                ...
                if outcome.category in ["duplication"] and getattr(
                    outcome, "guide_mismatch", -1
                ):
                    duplication_outcome = (
                        prime_editing_layout.DuplicationOutcome.from_string(
                            outcome.details
                        )
                    )

                    for side, value in zip(
                        ["left", "right"], duplication_outcome.ref_junctions[0]
                    ):
    ...
    def donor_misintegration_info(self):
        ...
        with open(self.fns["filtered_donor_misintegration_details"]) as details_fh:
            for line in details_fh:
                outcome = pooled_layout.LongTemplatedInsertionOutcome.from_string(
                    line.strip()
                )
                outcomes.append(outcome)
        return outcomes
    ...
    def extract_genomic_insertion_info(self):
        ...
        with open(self.fns["filtered_genomic_insertion_seqs"], "w") as seqs_fh, open(
            self.fns["filtered_genomic_insertion_details"], "w"
        ) as details_fh:

            for outcome in self.outcome_iter():
                if (
                    outcome.category == "genomic insertion"
                    and outcome.subcategory == "hg19"
                ):
    ...
    def extract_deletion_ranges(self):
        deletion_ranges = ranges.Ranges.deletion_ranges(
            [self], with_edit=True, without_edit=True, exclude_buffer_around_primers=10
        )
...
class SingleGuideNoUMIExperiment(SingleGuideExperiment):
    ...
    def merge_read_chunks(self):
        ...
        if guide_primer.strand == "+":
            expected_primer = ti.target_sequence[
                guide_primer.start : guide_primer.end + 1
            ]
            expected_protospacer = self.pool.variable_guide_library.guides_df.loc[
                self.variable_guide, "protospacer"
            ]
        ...
        with gzip.open(
            self.fns_by_read_type["fastq"]["collapsed_R2"], "wt", compresslevel=1
        ) as combined_fh, gzip.open(
            self.fns_by_read_type["fastq"]["low_quality_R2"], "wt", compresslevel=1
        ) as low_quality_fh:
            ...
            for read in self.reads:
                ...
                for i, (seen, expected) in enumerate(
                    zip(annotation_in["guide"], expected_seq)
                ):
                    if seen != expected:
                        mismatches.append(str(i))

                annotation_out = Annotation_out(
                    query_name=annotation_in["query_name"],
    ...
    def category_counts(self):
        ...
        if self.outcome_counts is None:
            ...
            return (
                self.outcome_counts.xs(True)
                .groupby(level=["category", "subcategory"], sort=False)
                .sum()
            )
...
class CommonSequenceExperiment(SingleGuideExperiment):
    def __init__(
        self, base_dir, pool_name, fixed_guide, variable_guide, *args, **kwargs
    ):
        # Note: pool_name is an anachronism
        full_pool_name = pool_name + (f"{fixed_guide}-{variable_guide}",)

        super().__init__(
            base_dir, full_pool_name, "common", "sequences", *args, **kwargs
        )
...
class PooledScreen:
    def __init__(self, base_dir, name, category_groupings=None, progress=None):
        ...
        self.fns = {
            ...
            "collapsed_total_outcome_counts": self.dir
            / "collapsed_total_outcome_counts.txt",
            "category_counts": self.dir / "category_counts.txt",
            "subcategory_counts": self.dir / "subcategory_counts.txt",
            "high_frequency_outcome_counts": self.dir
            / "high_frequency_outcome_counts.hdf5",
            ...
            "filtered_templated_insertion_details": self.dir
            / "filtered_templated_insertion_details.hdf5",
            "filtered_duplication_details": self.dir
            / "filtered_duplication_details.hdf5",
            "deletion_ranges": self.dir / "deletion_ranges.hdf5",
            "duplication_ranges": self.dir / "duplication_ranges.hdf5",
            "genomic_insertion_length_counts": self.dir
            / "genomic_insertion_length_counts.txt",
            "genomic_insertion_length_fractions": self.dir
            / "genomic_insertion_length_fractions.txt",
            "highest_guide_correlations": self.dir / "highest_guide_correlations.txt",
            "gene_level_category_statistics": self.dir
            / "gene_level_category_statistics.txt",
    ...
    def variable_guide_library(self):
        return guide_library.GuideLibrary(
            self.base_dir, self.sample_sheet["variable_guide_library"]
        )
    ...
    def fixed_guide_library(self):
        if "fixed_guide_library" in self.sample_sheet:
            return guide_library.GuideLibrary(
                self.base_dir, self.sample_sheet["fixed_guide_library"]
            )
        else:
    ...
    @memoized_property
    def supplemental_indices(self):
        all_supplemental_indices = target_info.locate_supplemental_indices(
            self.base_dir
        )
        return {
            name: all_supplemental_indices[name]
            for name in self.supplemental_index_names
        }
    ...
    def guide_combinations_by_read_count(self):
        if self.fns["read_counts"].exists():
            read_counts = pd.read_csv(
                self.fns["read_counts"], sep="\t", index_col=[0, 1]
            ).squeeze()
            return [
                (fg, vg)
                for fg, vg in read_counts.index.values
                if fg != "unknown" and vg != "unknown"
            ]
        else:
    ...
    def guide_combinations_for_gene(self, gene, **kwargs):
        ...
        for fixed_guide in self.fixed_guide_library.gene_guides(gene, **kwargs):
            for variable_guide in self.variable_guide_library.gene_guides(
    ...
    def single_guide_experiments(self, no_progress=False):
        for fixed_guide, variable_guide in self.guide_combinations:
            yield self.single_guide_experiment(
                fixed_guide, variable_guide, no_progress=no_progress
            )
    ...
    def generate_outcome_counts(self):
        ...
        for exp in self.progress(
            exps, total=len(self.guide_combinations), desc=description
        ):
            ...
            if counts is None:
                ...
                collapsed = pd.concat(
                    {
                        pg: collapse_categories(counts.xs(pg))
                        for pg in [True, False]
                        if pg in counts.index.levels[0]
                    }
                )
        ...
        counts = scipy.sparse.dok_matrix(
            (len(outcome_order), len(self.guide_combinations)), dtype=int
        )
    ...
    def copy_snapshot(
        ...
        new_sample_sheet_fn.write_text(
            yaml.safe_dump(sample_sheet, default_flow_style=False)
        )
    ...
    def outcome_counts_df(self, *, collapsed=True, snapshot_name=None):
        ...
        df = pd.DataFrame(
            ...
            index=self.total_outcome_counts(
                collapsed=collapsed, snapshot_name=snapshot_name
            ).index,
    ...
    def outcome_counts_raw(self, *, guide_status="perfect", snapshot_name=None):
        ...
        if guide_status == "all":
            outcome_counts = (
                self.outcome_counts_df(collapsed=True, snapshot_name=snapshot_name)
                .groupby(level=[1, 2, 3])
                .sum()
            )
        else:
            perfect_guide = guide_status == "perfect"
            all_counts = self.outcome_counts_df(
                collapsed=True, snapshot_name=snapshot_name
            )
    ...
    def outcome_counts(self, *, guide_status="perfect", snapshot_name=None):
        outcome_counts = self.outcome_counts_raw(
            guide_status=guide_status, snapshot_name=snapshot_name
        )
        ...
        gi_length_counts = self.genomic_insertion_length_counts.loc["hg19"].drop(
            ALL_NON_TARGETING, level=1
        )
        ...
        if ("genomic insertion", "hg19", "collapsed") in outcome_counts.index:
            if guide_status == "perfect":
                if not np.allclose(
                    short_and_long,
                    outcome_counts.loc["genomic insertion", "hg19", "collapsed"],
                ):
                    # TODO: understand source of discrepancies here
                    pass

            outcome_counts.drop(
                ("genomic insertion", "hg19", "collapsed"), inplace=True
            )

            outcome_counts.loc[
                "genomic insertion", "hg19", f"<={length_cutoff} nts"
            ] = short_gis
            outcome_counts.loc[
                "genomic insertion", "hg19", f">{length_cutoff} nts"
            ] = long_gis
    ...
    def generate_high_frequency_outcome_counts(self):
        ...
        to_write = {
            ...
            "log2_fold_changes": self.log2_fold_changes_for_all_fixed_guides().loc[
                outcomes
            ],
        }
        ...
        with h5py.File(self.fns["high_frequency_outcome_counts"], "w") as fh:
            for key, df in to_write.items():
                ...
                for level in df.index.names:
                    dataset.attrs[level] = [
                        s.encode() for s in df.index.get_level_values(level)
                    ]

                for level in df.columns.names:
                    dataset.attrs[level] = [
                        s.encode() for s in df.columns.get_level_values(level)
                    ]

            UMI_counts = fh.create_dataset("UMI_counts", data=self.UMI_counts().values)
            UMI_counts.attrs["variable_guide"] = [
                s.encode() for s in self.UMI_counts().index
            ]
    ...
    def load_high_frequency_outcome_counts(self, key):
        with h5py.File(self.fns["high_frequency_outcome_counts"]) as fh:
            ...
            index = pd.MultiIndex.from_tuples(
                zip(*index_levels), names=index_level_names
            )

            columns_levels = [dataset.attrs[level] for level in columns_level_names]
            columns = pd.MultiIndex.from_tuples(
                zip(*columns_levels), names=columns_level_names
            )
    ...
    @memoized_with_kwargs
    def outcomes_above_simple_threshold(
        self, *, frequency_threshold=0.01, use_high_frequency_counts=False
    ):
    ...
    @memoized_with_kwargs
    def outcomes_with_N_expected_in_quantile(
        self, *, n=15, q=0.25, use_high_frequency_counts=False
    ):
        if use_high_frequency_counts:
        ...
        outcomes = self.outcomes_above_simple_threshold(
    ...
    def canonical_outcomes(self, *, use_high_frequency_counts=False):
        return self.outcomes_with_N_expected_in_quantile(
            n=15, q=0.25, use_high_frequency_counts=use_high_frequency_counts
        )

    def active_guides_above_multiple_of_max_nt(
        self, relevant_outcomes, multiple, use_high_frequency_counts=False
    ):
        chi_squared = self.chi_squared_per_guide(
    ...
    def top_n_active_guides(
        self, relevant_outcomes, n, use_high_frequency_counts=False
    ):
        chi_squared = self.chi_squared_per_guide(
            relevant_outcomes=relevant_outcomes,
    ...
    def canonical_active_guides(self, *, use_high_frequency_counts=False):
        return self.active_guides_above_multiple_of_max_nt(
            self.canonical_outcomes(
                use_high_frequency_counts=use_high_frequency_counts
            ),
            2,
            use_high_frequency_counts=use_high_frequency_counts,
    ...
    def high_frequency_log2_fold_change_intervals(self):
        intervals = {
            k: self.load_high_frequency_outcome_counts(
                f"log2_fold_changes_interval_{k}"
            )
    ...
    def UMI_counts_from_high_frequency_counts(self):
        with h5py.File(self.fns["high_frequency_outcome_counts"]) as fh:
            ...
            index = pd.Index(
                data=dataset.attrs["variable_guide"], name="variable_guide"
            )
    ...
    def compute_deletion_boundaries(self):
        ti = self.target_info
        deletion_fractions = self.outcome_fractions("perfect").xs(
            "deletion", drop_level=False
        )
        ...
        for (c, s, d), row in self.progress(deletion_fractions.iterrows()):
            ...
            deletion = (
                pooled_layout.DeletionOutcome.from_string(d)
                .perform_anchor_shift(-ti.anchor)
                .deletion
            )
        ...
        deletion_boundaries.to_hdf(
            self.fns["deletion_boundaries"], "deletion_boundaries"
        )
    ...
    def fraction_removed_log2_fold_changes(self):
        ...
        with warnings.catch_warnings():
            ...
            log2_fold_changes = np.log2(
                fraction_removed.div(
                    fraction_removed[ALL_NON_TARGETING, ALL_NON_TARGETING], axis=0
                )
            )
    ...
    @memoized_with_kwargs
    def UMI_counts_for_all_fixed_guides(
        self, *, guide_status="perfect", snapshot_name=None
    ):
        return self.outcome_counts_raw(
            guide_status=guide_status, snapshot_name=snapshot_name
        ).sum()

    @memoized_with_kwargs
    def UMI_counts(
        self, *, guide_status="perfect", fixed_guide="none", snapshot_name=None
    ):
        return self.UMI_counts_for_all_fixed_guides(
            guide_status=guide_status, snapshot_name=snapshot_name
        )[fixed_guide]

    @memoized_with_kwargs
    def outcome_fractions(self, *, guide_status="perfect", snapshot_name=None):
        per_guide_fractions = self.outcome_counts(
            guide_status=guide_status, snapshot_name=snapshot_name
        ) / self.UMI_counts_for_all_fixed_guides(
            guide_status=guide_status, snapshot_name=snapshot_name
        )

        all_nt_fractions = [
            self.non_targeting_fractions(
                guide_status=guide_status,
                fixed_guide=fixed_guide,
                snapshot_name=snapshot_name,
            )
            for fixed_guide in list(self.fixed_guides) + [ALL_NON_TARGETING]
        ]
    ...
    def extract_category_counts(self):
        ...
        subcategory_counts = (
            ...
            .groupby(level=["category", "subcategory"])
            .sum()
        )
        subcategory_counts = subcategory_counts.drop(
            "malformed layout", errors="ignore"
        )
        subcategory_counts[ALL_NON_TARGETING] = subcategory_counts[nt_guides].sum(
            axis=1
        )
    ...
    def gene_level_category_statistics(self):
        return pd.read_csv(
            self.fns["gene_level_category_statistics"], header=[0, 1], index_col=0
        )
    ...
    def subcategory_counts(self):
        subcategory_counts = pd.read_csv(
            self.fns["subcategory_counts"], index_col=[0, 1]
        )
    ...
    def category_fractions(self):
        ...
        if self.category_groupings is not None:
            only_relevant_cats = pd.Index.difference(
                fs.index, self.category_groupings["not_relevant"]
            )
            relevant_but_not_specific_cats = pd.Index.difference(
                only_relevant_cats, self.category_groupings["specific"]
            )
            ...
            relevant_but_not_specific = only_relevant_normalized.loc[
                relevant_but_not_specific_cats
            ].sum()
    ...
    def categories_by_baseline_frequency(self):
        return (
            self.category_fractions[ALL_NON_TARGETING]
    ...
    def category_log2_fold_changes(self):
        fold_changes = self.category_fractions.div(
            self.category_fractions[ALL_NON_TARGETING], axis=0
        )
        return np.log2(fold_changes)

    @memoized_property
    def category_fraction_differences(self):
        return self.category_fractions.sub(
            self.category_fractions[ALL_NON_TARGETING], axis=0
        )
    ...
    def subcategories_by_baseline_frequency(self):
        return (
            self.subcategory_fractions[ALL_NON_TARGETING]
            .sort_values(ascending=False)
            .index.values
        )
    ...
    def subcategory_log2_fold_changes(self):
        fold_changes = self.subcategory_fractions.div(
            self.subcategory_fractions[ALL_NON_TARGETING], axis=0
        )
        return np.log2(fold_changes)

    @memoized_property
    def subcategory_fraction_differences(self):
        return self.subcategory_fractions.sub(
            self.subcategory_fractions[ALL_NON_TARGETING], axis=0
        )

    @memoized_with_kwargs
    def non_targeting_counts(
        self,
        *,
    ...
    @memoized_with_kwargs
    def non_targeting_fractions(
        self, *, guide_status="perfect", fixed_guide="none", snapshot_name=None
    ):
        counts = self.non_targeting_counts(
            guide_status=guide_status,
            fixed_guide=fixed_guide,
            snapshot_name=snapshot_name,
        )
    ...
    @memoized_with_kwargs
    def most_frequent_outcomes(
        self, *, guide_status="all", fixed_guide="none", use_high_frequency_counts=False
    ):
        if use_high_frequency_counts:
            vals = self.high_frequency_outcome_fractions[ALL_NON_TARGETING]
        else:
            vals = self.non_targeting_counts(
                guide_status=guide_status, fixed_guide=fixed_guide
            )
    ...
    def common_counts(self, *, guide_status="perfect"):
        ...
        leftover = (
            self.UMI_counts_for_all_fixed_guides(guide_status=guide_status)
            - common_counts.sum()
        )
        leftover_row = pd.DataFrame.from_dict(
            {("uncommon", "uncommon", "collapsed"): leftover}, orient="index"
        )
    ...
    def common_fractions(self, *, guide_status="perfect"):
        return self.common_counts(guide_status=guide_status) / self.UMI_counts(
            guide_status=guide_status
        )
    ...
    @memoized_with_kwargs
    def log2_fold_changes_for_all_fixed_guides(
        self, *, guide_status="perfect", fixed_guide="none", snapshot_name=None
    ):
    ...
    @memoized_with_kwargs
    def log2_fold_changes(
        self, *, guide_status="perfect", fixed_guide="none", snapshot_name=None
    ):
        return self.log2_fold_changes_for_all_fixed_guides(
            guide_status=guide_status,
            fixed_guide=fixed_guide,
            snapshot_name=snapshot_name,
        )[fixed_guide]
    ...
    def canonical_log2_fold_changes(self):
        return self.log2_fold_changes().loc[
            self.canonical_outcomes, self.canonical_active_guides
        ]

    def log2_fold_changes_multiple_outcomes(
        self, outcomes, fixed_guide="none", guide_status="perfect"
    ):
        fractions = (
            self.outcome_fractions(guide_status)[fixed_guide].loc[outcomes].sum(axis=0)
        )
        nt_fraction = (
            self.non_targeting_fractions(guide_status, fixed_guide).loc[outcomes].sum()
        )
    ...
    def donor_outcomes_containing_SNV(self, SNV_name):
        ...
        outcomes = [
            (c, s, d)
            for c, s, d in nt_fracs.index.values
            if c == "donor" and d[SNV_index] == donor_base
        ]
        return outcomes
    ...
    def conversion_fractions(self):
        ...
        conversion_fractions = pd.DataFrame.from_dict(
            conversion_fractions, orient="index"
        ).sort_index()
    ...
    def conversion_log2_fold_changes(self):
        ...
        fold_changes = fractions.div(fractions[ALL_NON_TARGETING], axis="index").drop(
            columns=[ALL_NON_TARGETING]
        )
        log2_fold_changes = np.log2(fold_changes)
        return log2_fold_changes
    ...
    def outcomes_containing_specific_mismatch(self):
        ...
        for c, s, d in self.non_targeting_fractions_full_arguments(
            "perfect", "none"
        ).index.values:
            if c == "mismatches":
                outcome = pooled_layout.MismatchOutcome.from_string(
                    d
                ).undo_anchor_shift(self.target_info.anchor)
    ...
    def sort_outcomes_by_gene_phenotype(
        self, outcomes, gene, top_n=None, ascending=False
    ):
        if top_n is None:
    ...
    def rational_outcome_order(
        ...
        def get_deletion_info(details):
            ...
            return {
                "num_MH_nts": len(deletion.starts_ats) - 1,
                "start": min(deletion.starts_ats),
        ...
        conditions = {
            ...
            "complex templated insertion": lambda c, sc, d: c
        ...
        most_frequent_outcomes = self.most_frequent_outcomes(
            fixed_guide=fixed_guide, use_high_frequency_counts=use_high_frequency_counts
        )[:num_outcomes]
        groups = {}
    ...
    def merge_common_sequence_special_alignments(self):
        ...
        if len(all_fns) > 0:
            sam.merge_sorted_bam_files(
                all_fns, self.fns["common_sequence_special_alignments"]
            )
    ...
    def merge_reads_per_UMI(self):
        ...
        for exp in self.progress(
            self.single_guide_experiments(), desc=description, total=total
        ):
    ...
    def merge_templated_insertion_details(
        self, fn_key="filtered_templated_insertion_details"
    ):
        with h5py.File(self.fns[fn_key], "w") as merged_f:
            ...
            for exp in self.progress(
                ...
                def add_to_merged(key, dataset):
                    ...
                    if len(fields) == 3:
                        all_counts[key].update(
                            dict(zip(dataset["values"], dataset["counts"]))
                        )
    ...
    def extract_genomic_insertion_length_distributions(self):
        ...
        all_guide_combos[
            (ALL_NON_TARGETING, ALL_NON_TARGETING)
        ] = self.non_targeting_guide_pairs

        for fg in self.fixed_guides:
            all_guide_combos[(fg, ALL_NON_TARGETING)] = self.guide_plus_non_targeting(
                fg
            )
        ...
        for guide_name, guide_combos in self.progress(all_guide_combos.items()):
            for organism in ["hg19", "bosTau7"]:
                lengths, counts = self.templated_insertion_details(
                    guide_combos, "genomic insertion", organism, "insertion_length"
                )
    ...
    def genomic_insertion_length_counts(self):
        df = pd.read_csv(
            self.fns["genomic_insertion_length_counts"], index_col=[0, 1, 2]
        ).astype(int)
    ...
    def genomic_insertion_length_distributions(self):
        ...
        """
        df = pd.read_csv(
            self.fns["genomic_insertion_length_fractions"], index_col=[0, 1, 2]
        )
        df.columns = [int(c) for c in df.columns]
        df.index.names = ["organism", "fixed_guide", "variable_guide"]
        df.drop("eGFP_NT2", level="variable_guide", inplace=True, errors="ignore")
    ...
    def templated_insertion_details(
        self,
        guide_pairs,
        category,
        subcategories,
        field,
        fn_key="filtered_templated_insertion_details",
    ):
        counts = Counter()
    ...
    def merge_deletion_ranges(self):
        ...
        for side in ["start", "end"]:
            edge_counts = {
                guide: ranges[guide].edge_counts["start"] for guide in ranges
            }
    ...
    def deletion_ranges(self):
        deletion_ranges = {
            key: pd.read_hdf(self.fns["deletion_ranges"], key=key)
            for key in ["start", "end", "total_reads"]
        }
    ...
    def merge_duplication_ranges(self):
        ranges = {}
        for guide in self.progress(self.variable_guide_library.guides):
            ranges[guide] = self.single_guide_experiment(
                "none", guide
            ).duplication_ranges
    ...
    def duplication_ranges(self):
        duplication_ranges = {
            key: pd.read_hdf(self.fns["duplication_ranges"], key=key)
            for key in ["start", "end", "total_reads"]
        }
    ...
    def chi_squared_per_guide(
        ...
        if isinstance(relevant_outcomes, int):
            relevant_outcomes = self.most_frequent_outcomes(fixed_guide=fixed_guide)[
                :relevant_outcomes
            ]
        ...
        if exclude_unedited:
            relevant_outcomes = [
                ...
                if outcome != ("wild type", "clean", "n/a")
            ]
        ...
        non_targeting_guides = sorted(
            set(self.variable_guide_library.non_targeting_guides) & set(nonzero_guides)
        )
        ...
        expected = pd.DataFrame(
            np.outer(nt_fractions, UMI_counts),
            index=counts.index,
            columns=nonzero_guides,
        )
        ...
        if only_best_promoter:
            chi_squared = chi_squared[
                self.variable_guide_library.guides_df["best_promoter"]
            ]
    ...
    def compute_highest_guide_correlations(self):
        ...
        outcomes = [
            (c, s, d)
            for c, s, d in self.most_frequent_outcomes("none")
            if c not in ["uncategorized"]
        ][:40]
        ...
        for i in self.progress(range(len(guide_to_gene))):
            ...
            for j in range(i + 1, len(guide_to_gene)):
                ...
                highest_gene_gene[gene_pair] = max(
                    r, highest_gene_gene.get(gene_pair, -1)
                )
    ...
    def process(self, num_processes=18):
        ...
        def process_stage(stage):
            ...
            for exp_i, (fixed_guide, variable_guide) in enumerate(
                self.guide_combinations_by_read_count
            ):
                arg_tuple = (
                    self.base_dir,
                    self.name,
...
class MergedPools(PooledScreen):
    def __init__(
        self, base_dir, merged_name, pool_names, category_groupings=None, progress=None
    ):
        super().__init__(base_dir, merged_name, progress=progress)

        self.pool_names = pool_names
        self.pools = {
            pn: PooledScreen(base_dir, pn, category_groupings, progress)
    ...
    def merge_outcome_counts(self):
        all_counts = {
            group: pool.outcome_counts_df(True) for group, pool in self.pools.items()
        }
        ...
        merged_counts.sum(axis=1).to_csv(
            self.fns["collapsed_total_outcome_counts"], header=False
        )
    ...
    def merge_category_counts(self):
        ...
        subcategory_counts = sum(
            pool.subcategory_counts for pn, pool in self.pools.items()
        )
        subcategory_counts.to_csv(self.fns["subcategory_counts"])
    ...
    if sample_sheet_fn.exists():
        ...
        if pooled:
            if not sample_sheet.get("has_UMIs", True):
                ...
                pool = MergedPools(
                    *args, groups=sample_sheet["pools_to_merge"], **kwargs
                )
            else:
...
def get_all_pools(
    base_dir=Path.home() / "projects" / "repair_seq",
    ...
    for pool_dir in pool_dirs:
        name = pool_dir.name
        pool = get_pool(
            base_dir, name, category_groupings=category_groupings, progress=progress
        )
        if pool is not None:
            pools[name] = pool
...
def process_single_guide_experiment_stage(
    base_dir,
    pool_name,
    fixed_guide,
    variable_guide,
...
class PooledScreenExplorer(explore.Explorer):
    ...
    def set_up_read_selection_widgets(self):
        self.widgets.update(
            {
                "guide": ipywidgets.Select(
                    options=self.guides,
                    value=self.initial_guide,
                    layout=ipywidgets.Layout(height="200px", width="300px"),
                ),
            }
        )
